...
static void testDeletedRecords(void);
...
void testDeletedRecords(void) {
  ...
  int numInserts = 10, numDeletes = 5, numFinal = 10, i;
  Record *r;
  RID *rids;
  Schema *schema;
  testName = "test creating a new table and delete tuples";
  schema = testSchema();
  rids = (RID *) malloc(sizeof(RID) * numInserts);
